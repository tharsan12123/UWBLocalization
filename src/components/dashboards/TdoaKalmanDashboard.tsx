...
import { MapPin, TrendingUp, Filter, Target } from 'lucide-react';
import useKalmanLocalizationData from '../../useKalmanLocalizationData';

export const TdoaKalmanDashboard: React.FC = () => {
  const [isTracking, setIsTracking] = useState(false);
  const [zoom, setZoom] = useState(0.7);
  const kalmanData = useKalmanLocalizationData();
  const [kalmanGain, setKalmanGain] = useState(0.8);
  ...
  const localizationData = useKalmanLocalizationData();

  const tag = kalmanData?.tag_kalman ?? [0, 0];
  const f_tag = kalmanData?.tag_kalman?? [0,0];
  const tdoaValues = kalmanData?.tag_tdoa ?? { tdoa12: 0, tdoa13: 0, tdoa23: 0 };
  const anchors = kalmanData?.anchors ?? {};
  const accuracy = kalmanData?.accuracy ?? 0;
  ...
  const metrics = [
    { label: 'Raw X Position', value: `${tag[0].toFixed(2)} m`, icon: MapPin },
    { label: 'Raw Y Position', value: `${tag[1].toFixed(2)} m`, icon: MapPin },
    { label: 'Filtered X Position', value: `${f_tag[0].toFixed(2)} m`, icon: Filter },
    { label: 'Filtered Y Position', value: `${f_tag[1].toFixed(2)} m`, icon: Filter },
    { label: 'Kalman Gain', value: kalmanGain.toFixed(2), icon: TrendingUp },
    { label: 'Accuracy', value: `±0.05 m`, icon: Target },
  ];
